...
use super::{d3d, d3d11, dxgi, misc, Interface, BOOL, HMODULE, HWND};
...
impl Adapter {
	...
	pub(crate) unsafe fn set_swap_chain(
	...
	pub(crate) unsafe fn present(&mut self, rgba: &[f32; 4]) -> Result<(), windows::core::Error> {
		...
		if let Some(rtv) = &self.back_buffer_rtv {
			self.context
